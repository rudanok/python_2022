# Задание 2a. Создайте программу для игры с конфетами против умного бота.
...
# В отличие от предыдущего варианта бот каждый раз берёт число конфет, равное остатку от деления на 29.
...
def input_number(player_name, player_number, amount):
    if player_number == 1: # бот
        ...
        print(f'{player_name}, сколько конфет вы берёте (подсказка: {amount%29}):', end = ' ')
        x = int(input())
...
while amount > 28:
    if player_number == 1:
        q = input_number('Бот', player_number, amount)
        player_number = 2
    else:
        q = input_number(user_name, player_number, amount)
        player_number = 1
    amount -= q
